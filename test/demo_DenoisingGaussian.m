function y_hat = demo_DenoisingGaussian(input)
...
% h_opt_Q : adaptive directional scales
    ...
    % >>> CHANGE: do NOT add noise to the input
    addnoise = 0;                % add noise to observation (0 = NO)
    sigma_noise = 0.1;           % standard deviation of the noise (unused when addnoise=0)
    ...
    %--------------------------------------------------------------------------
    h1 = [1 2 3 5 7 11];
    h2 = max(1, ceil(h1*tan(0.5*pi/directional_resolution)));  % same length as h1
    lenh = length(h1);
    ...
    TYPE = 10;          % TYPE IS A SYMMETRY OF THE WINDOW
                        ...
                        %
                        % rotation variants: 110/111 exact sampling, 210/211 bilinear, etc.
    ...
    if addnoise == 1
        ...
        z = y;  % <<< input is used directly as observation (no added noise)
    end

    sigma = function_stdEst(z);   % estimates noise standard deviation
    ...
    sigmaiter = repmat(sigma, size_z_1, size_z_2); %#ok<NASGU>
    stop_condition = 0; %#ok<NASGU>
    toc
    ...
    for s1 = 1:directional_resolution     % directional index
        for s2 = 1:lenh     % kernel size index
            gha = kernels_higher_order{s1, s2, 1}(:, :, 1);   % gets single kernel
            ghb = kernels_higher_orderb{s1, s2, 1}(:, :, 1);
            gh = (1 + sharparam) * ghb - sharparam * gha;

            ghorigin(s1, s2) = gh((end+1)/2, (end+1)/2); %#ok<AGROW>
            ...
            stdh(:, :, s2) = repmat(sigma * (sum(gh(:).^2))^0.5, size_z_1, size_z_2);  % Std of estimate
        end
        ...
        [Err_y_Q, Err_labels] = function_Errors(y, YICI, z);  %#ok<NASGU>
        ...
        if s1 == 1
            ...
            tab_content = [['Dir.# ', repmat(' ', [1 size(Err_labels,2)-6]); Err_labels], ...
                           repmat(' ', [size(Err_y_Q,1)+1 1]), ...
                           [repmat(' ', [1 floor((size(Err_y_Q_str,2))/2)]), num2str(s1), repmat(' ', [1, ceil((size(Err_y_Q_str,2))/2)-1]); Err_y_Q_str], ...
                           Err_Wait];
        else
            Err_y_Q_str = num2str(Err_y_Q, number_of_digits);
            disp(sprintf(repmat('\b', [1 numel(tab_content) + size(tab_content,1) + 32])))
            tab_content = [tab_content(:, 1:end-size_Err_Wait), repmat(' ', [size(Err_y_Q,1)+1 2]), ...
                           [repmat(' ', [1 1-numel(num2str(s1))+floor((size(Err_y_Q_str,2))/2)]), num2str(s1), repmat(' ', [1, ceil((size(Err_y_Q_str,2))/2)-1]); Err_y_Q_str], ...
                           Err_Wait];
        end
        if s1 == 1
            disp(sprintf(repmat('\b', [1 32])))
        end
        if s1 < directional_resolution
        ...
        disp(tab_content);
        %%%% END RESULTS TABLE DISPLAY
    end

    disp(' ')
    disp('Filtering completed.         ')
    YICI_Final1 = YICI_Final1 ./ var_inv;
    YICI_Final2 = (YICI_Final2 + z .* CWW / directional_resolution) ./ (var_inv - CWW + CWW ./ directional_resolution);
    ...
    figure
    subplot(1,2,1)
    imshow(z); title('Observation (input, no added noise)')
    subplot(1,2,2)
