...
%
%
% Alessandro Foi, July 5, 2023 (updated version of 6 march 2004 to make it compatible with newer Matlab versions)

function [G, G1, index_polynomials]=function_LPAKernelMatrixTheta(h2,h1,window_type,sig_wind,TYPE,theta, m)
global beta

if exist('imrotate_old')
use_imrotate_old=true;
else
use_imrotate_old=false;
end

if TYPE<100
    interpMode='nearest';     % use 'nearest' or 'bilinear' for different interpolation schemes ('bicubic'...?)
elseif (TYPE>=200)&&(TYPE<300)
    interpMode='bilinear';     % use 'nearest' or 'bilinear' for different interpolation schemes ('bicubic'...?)
else % if TYPE>=300
interpMode='bicubic';     % use 'nearest' or 'bilinear' for different interpolation schemes ('bicubic'...?)
end
...
if (theta~=0)&&use_imrotate_old
    win_fun=imrotate_old(win_fun1,theta*180/pi,interpMode); 
end
if (theta~=0)&&~use_imrotate_old
    win_fun=imrotate(win_fun1,theta*180/pi,interpMode); 
end
